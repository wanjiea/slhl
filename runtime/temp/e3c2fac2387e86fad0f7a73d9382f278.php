<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:89:"E:\phpstudy8\phpstudy_pro\WWW\slhl\public/../application/admin\view\cases\cases_list.html";i:1568896756;s:72:"E:\phpstudy8\phpstudy_pro\WWW\slhl\application\admin\view\Base\base.html";i:1567050636;}*/ ?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
							...
							<div class="panel-body">
								...
								<div class="ibox-content-del">
									...
									<div class="row">
											...
											<div class="col-sm-1">
												<div class="input-group">
													<select name='cate_id' class='form-control input-sm' style="font-size:12px;width:115%">
														<option value='0' style="width:100%">选择案例分类</option>
										...
										<a href="<?php echo url('Cases/CasesAdd'); ?>" class="btn btn-sm btn-primary">添加案例</a>
									</div>
									...
									<div class="table-responsive">
										<table class="table table-hover products_table">
											...
											<tr >
												...
												<td>
													<?php if(is_array($cate_list) || $cate_list instanceof \think\Collection || $cate_list instanceof \think\Paginator): if( count($cate_list)==0 ) : echo "" ;else: foreach($cate_list as $key=>$voo): if($voo['id'] == $vo['cate_id']): ?>
										            <?php echo $voo['class_name']; endif; endforeach; endif; else: echo "" ;endif; ?>
												</td>
												<td><?php echo $vo['address']; ?></td>
												<td><?php echo $vo['order_num']; ?></td>
												<td><?php echo $vo['income']; ?></td>
												<td><?php echo $vo['cycle']; ?></td>
												<td><?php echo $vo['return']; ?></td>
												...
												<td>
													<a href="<?php echo url('Cases/CasesAdd', ['cases_id' => $vo['id']]); ?>" class="btn btn-xs btn-warning">编辑</a>
											...
											<tr>
												<td colspan="12" class="pro_lasttd">
													...
													<div class="pro_table_contro">
														...
														<select id="Cases_cate_id" style="font-size:12px;margin-left: 20px;padding: 1px 5px;height:22px;">
															<option value='0' style="width:100%">选择案例分类</option>
															<?php if(is_array($cate_list) || $cate_list instanceof \think\Collection || $cate_list instanceof \think\Paginator): if( count($cate_list)==0 ) : echo "" ;else: foreach($cate_list as $key=>$vo): ?>
															<option value='<?php echo $vo['id']; ?>' <?php if($vo['id'] == $cate_id): ?>selected="selected"<?php endif; ?>><?php echo $vo['class_name']; ?></option>
															<?php endforeach; endif; else: echo "" ;endif; ?>
...
<script type="text/javascript">
	...
	$('.btn_change').click(function () {
		...
		var Cases_cate_id = $('#Cases_cate_id').val();
		dialog.showTips('确定转移吗？', "firm", function(){

			$.ajax({
				url: "<?php echo url('Cases/CasesChange'); ?>",
				type: "post",
				dataType: "json",
				data: {
					id: id,
					Cases_cate_id: Cases_cate_id,
				}
			}).done(function (res) {
				if (res.status == 1) {
	...
	$('.btn_del').click(function () {
		...
		dialog.showTips('确定要删除吗？', "firm", function(){
			...
			$.ajax({
				url: "<?php echo url('Cases/delCases'); ?>",
				type: "post",
				dataType: "json",
				data: {
					id: id,
	...
	function status (id,item, obj) {
		...
		$.ajax({
			type:"POST",
			url: 'http://www.testslhl.com/admin/Cases/CasesStatus',
